...
└── tokens/                                      # 디자인 시스템 토큰 — "어떻게 코드로 쓸지"
    ...
    ├── figma-variable-mapping.md                # Figma 변수 → Tailwind 클래스 매핑
    ...
    ├── app-theme-tokens.md                      # 앱 테마 및 shadcn/Radix 호환 토큰
...
| `tokens/figma-variable-mapping.md` | Figma 힌트 코드 → Tailwind 변환 | Figma 작업 시 |
...
| 입력 | 사용 스킬 | 동작 |
|------|---------|------|
| Figma URL | `figma-to-code` | Figma 디자인 읽기 → 토큰 변환 → React/Tailwind 코드 생성 |
...
- React + TypeScript + Tailwind 기준으로 구현하고, UI primitive는 shadcn/ui 또는 Radix 기반 컴포넌트를 우선 사용
- hex 단독 코딩 금지 (참조용으로만 사용)
- Edge Case (Loading Skeleton / Empty State / Error) 필수 구현
- `cn()` 클래스 병합, 화살표 함수형 컴포넌트, Tailwind 토큰 클래스 적용
